...
export const orderService = {
  ...
  async createPaymobOrder(data: CreateOrderDto): Promise<PaymobOrderResponse> {
    const res = await axiosClient.post<PaymobOrderResponse>('/orders/paymob', data);
    return res.data;
  },
  ...
  async refundOrder(id: string, refundReason?: string): Promise<{ message: string }> {
    const res = await axiosClient.post(`/orders/${id}/refund`, { refundReason });
    return res.data;
  },
